...
import { areVoicesReady, speak } from './speech.js';
...
function setupBackgroundImage() {
    $('#backgroundImage').on('change', function () {
        ...
        if (selectedImage === 'greenscreen') {
            ...
            $('body').css({
                backgroundImage: `url('/comms/images/background/${selectedImage}')`,
...
// Direct HTTP AI request - works without socket.io
export async function askAIRequest(payload) {
    ...
    try {
        ...
        const response = await fetch(`${backendUrl}/api/comms/ask`, {
            method: 'POST',
        ...
        const aiText = result.text || result.message || 'Sorry, I could not understand that.';

        // Show in chat
        appendMessageToChat('assistant', aiText);
        // Save to history
        addToHistory('assistant', aiText);
        // Speak the response using the unified speak function (handles lip-sync)
        speak(aiText);

    } catch (error) {
        console.error('AI request failed:', error);
        const errMsg = 'Connection error. Please ensure the backend is running on localhost:8000.';
        appendMessageToChat('assistant', errMsg);
...
const HISTORY_KEY = 'eduverse_comms_history';
