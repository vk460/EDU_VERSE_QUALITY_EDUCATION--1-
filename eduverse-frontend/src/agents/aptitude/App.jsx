...
async function callGemini(prompt) {
  try {
    const res = await fetch(`${CONFIG.BACKEND_URL}/api/aptitude/generate-custom`, {
      method: "POST",
...
function PracticeTab({ topic, subtopic, difficulty, streak, onAnswer, onHome, userProfile, setUserProfile }) {
  ...
  const fetchStreak = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${CONFIG.BACKEND_URL}/api/aptitude/streak`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleNext() {
    if (currentIndex >= streak - 1) {
      ...
      if (userProfile && xpEarned > 0) {
        fetch(`${CONFIG.BACKEND_URL}/api/aptitude/user/add-xp`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      try {
        const res = await fetch(`${CONFIG.BACKEND_URL}/api/aptitude/analyze-streak`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            topic,
